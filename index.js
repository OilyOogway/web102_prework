...
let totalBackers = GAMES_JSON.reduce((acc,backers) => {
...
function filterUnfundedOnly() {
    ...
    let amountLessThanGoal = GAMES_JSON.filter((game) =>{
        return game.pledged < game.goal;
    });

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(amountLessThanGoal);
}
...
function filterFundedOnly() {
    ...
    let amountMet = GAMES_JSON.filter((game) =>{
        return game.pledged >= game.goal;
    });

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(amountMet);
}
...
Currently, ${numUnfunded} ${numUnfunded < 2 ? "game remains": "games remain"} unfunded. We
need your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
var dis = document.createElement('p');
dis.innerHTML = displayStr
document.getElementById('description-container').append(dis);
/************************************************************************************
...
let topGameName = document.createElement('p');
topGameName.innerText = first.name;
firstGameContainer.append(topGameName);
// do the same for the runner up item
let secondGameName = document.createElement('p');
secondGameName.innerText = second.name;
